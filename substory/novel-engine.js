...
function isButtonElement(element) {
    ...
    return element.closest && (
        ...
        element.closest('.choice-container') ||
        element.closest('.unlock-modal')
    );
}
...
function isValidTapArea(x, y) {
    ...
    // 分岐モード中は無効
    if (isInBranchMode || isShowingUnlockContent) {
        return false;
    }
...
// 🌸========== 分岐システム：選択肢表示 ==========🌸
function showChoices(branchData) {
    console.log('🌸 [分岐] 選択肢表示開始:', branchData);
    ...
    // 背景とオーディオを設定
    if (branchData.bg) showBackground(branchData.bg);
    if (branchData.audio && (loadingAudioEnabled || audioEnabled)) {
        playAudio(branchData.audio);
    }
    
    // 通常のテキスト表示処理を使用
    const autoProgress = manageTextDisplay(branchData);
    if (autoProgress) {
        return;
    }
    
    setTimeout(() => {
        showTextContent(branchData);
    }, 50);
}
...
function showChoiceButtons(choices, callback) {
    ...
    choices.forEach((choice, index) => {
        ...
        btnWrapper.className = 'choice-button-wrapper';
        
        if (choice.description) {
            const desc = document.createElement('div');
            desc.className = 'choice-description';
            desc.textContent = choice.description;
            btnWrapper.appendChild(desc);
        }
        ...
        choiceElements.push(btnWrapper);
        
        // アニメーション遅延
        setTimeout(() => {
            btnWrapper.classList.add('appear');
        }, index * 200);
    });
...
function selectBranch(choice) {
    console.log('🌸 [分岐] ブランチ選択:', choice);
    
    // 選択ボタンを削除
    const choiceContainer = document.getElementById('choiceContainer');
    if (choiceContainer) {
        choiceContainer.classList.add('fade-out');
        setTimeout(() => {
    ...
    if (choice.jumpTo !== undefined) {
        currentTextIndex = choice.jumpTo;
        clearAllText();
...
// 🎁========== 分岐システム：アンロック表示 ==========🎁
function showUnlockContent(unlockData) {
    console.log('🎁 [分岐] アンロック表示:', unlockData);
    
    isShowingUnlockContent = true;
    ...
    // 背景とオーディオを設定
    if (unlockData.bg) showBackground(unlockData.bg);
    if (unlockData.audio && (loadingAudioEnabled || audioEnabled)) {
        playAudio(unlockData.audio);
    }
    
    // 通常のテキスト表示処理を使用
    const autoProgress = manageTextDisplay(unlockData);
    if (autoProgress) {
        return;
    }
    
    setTimeout(() => {
        showTextContent(unlockData);
        
        // アンロックモーダル表示（テキスト表示完了後）
        setTimeout(() => {
            showUnlockModal(unlockData);
        }, 2000);
    }, 50);
}

// 🎁========== アンロックモーダル表示 ==========🎁
function showUnlockModal(unlockData) {
    console.log('🎁 [分岐] アンロックモーダル表示:', unlockData);
    
    const modal = document.createElement('div');
    modal.className = 'unlock-modal';
    modal.innerHTML = `
        <div class="unlock-content">
            <div class="unlock-title">🎉 おめでとうございます！ 🎉</div>
            <div class="unlock-message">
                ${unlockData.achievement || 'ストーリーを完了しました！'}
            </div>
            <div class="unlock-actions">
                <button class="unlock-btn continue-btn" onclick="hideUnlockModal()">続ける</button>
                <button class="unlock-btn restart-btn" onclick="restartStory()">最初から</button>
            </div>
        </div>
    `;
...
function hideUnlockModal() {
    ...
    isShowingUnlockContent = false;
    
    // 次のコンテンツがあれば続行
    if (currentTextIndex < storyContent.length - 1) {
        currentTextIndex++;
        showNextText();
    }
}
...
function showNextText() {
    try {
        ...
        if (content.type === 'choice') {
            console.log('🌸 [分岐] 選択ノードを検出:', content);
            showChoices(content);
            return;
        }
