...
if (typeof storyContent !== 'undefined') {
    ...
    // 🔍========== ファイル存在チェック強化デバッグ ==========🔍
    console.log('🔍 [デバッグ] ファイル存在チェック開始...');
    
    // 背景ファイルチェック
    const usedBgs = [...new Set(storyContent.filter(item => item.bg).map(item => item.bg))];
    console.log('🖼️ [デバッグ] 使用されている背景ID:', usedBgs);
    
    // 音声ファイルチェック
    const usedAudios = [...new Set(storyContent.filter(item => item.audio).map(item => item.audio))];
    console.log('🔊 [デバッグ] 使用されている音声ファイル:', usedAudios);
...
// 🖼️========== 【PNG画像対応・強化版】背景管理 ==========🖼️
function showBackground(bgId) {
    if (!bgId) {
        console.log('⚠️ [背景] bgIdが未指定');
        return;
    }
    
    if (bgId === currentBgId) {
        console.log('📄 [背景] 同一背景のためスキップ:', bgId);
        return;
    }
    
    console.log('🖼️ [背景] 背景変更:', currentBgId, '→', bgId, 'PNG画像対応');
    ...
    requestAnimationFrame(() => {
        ...
        if (bg) {
            console.log('🎯 [背景] 背景要素発見:', bgId);
            
            // 動画の存在確認
            let video = bg.querySelector('video');
            if (video) {
                console.log('🎬 [背景] MP4動画:', video.querySelector('source').src);
                video.play().catch(e => console.log('🔇 [背景動画] 無音再生:', e.message));
            } else {
                console.log('⚠️ [背景] 動画が見つからない');
            }
            
            setTimeout(() => {
                bg.classList.add('active');
                bg.style.opacity = '1';
                bg.style.transition = 'opacity 1.5s ease-in-out'; // ゆったりとした切り替え
                console.log('✅ [背景画像] 表示成功:', bgId);
            }, 100); // 短い遅延でスムーズな切り替え
            
            currentBgId = bgId;
        } else {
            console.error('❌ [背景] 背景要素が見つからない:', bgId);
            console.log('🔍 [デバッグ] 存在する背景要素:', Array.from(document.querySelectorAll('[id^="bg"]')).map(el => el.id));
        }
    });
}
...
function showNextText() {
    try {
        ...
        console.log('📖 [デバッグ] showNextText実行 - インデックス:', currentTextIndex, '/ 総数:', storyContent.length);
...
function startAfterLoading() {
    ...
    setTimeout(() => {
        ...
        if (!loadingAudioEnabled && typeof storyContent !== 'undefined' && storyContent[0] && storyContent[0].audio) {
            audioNotice.style.display = 'flex';
        }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // PNG画像背景の場合、動画設定は不要
    console.log('🖼️ [PNG背景] PNG画像背景モードで初期化（動画設定スキップ）');
    ...
    function checkLoadingCompletion() {
        if (loadingComplete && (firstAudioPreloaded || (typeof storyContent === 'undefined' || !storyContent[0]?.audio))) {
    ...
    // 最初の音声ファイルを事前読み込み
    if (typeof storyContent !== 'undefined' && storyContent[0]?.audio) {
